...
def main(argv):
    ...
    for location in locations['serviceLocations']:
       ...
       start = int(time.mktime(datetime.datetime.now().replace(year=2017,month=1,day=1,hour=0,minute=0,second=0,microsecond=0).timetuple()) * 1000)
       
       consumption = c.get_consumption(locationid,start,end,3)['consumptions']
       ...
       for month in consumption:
          print('\"%s\" ; \"%s\" ; \"Wh consumed\" ;  \"%s\" ; \"Wh Solar\"' % (datetime.datetime.fromtimestamp(month['timestamp']/1000),month['consumption'],month['solar']))
       #print('---')
